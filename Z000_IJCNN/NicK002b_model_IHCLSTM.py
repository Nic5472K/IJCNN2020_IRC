...
#---
# Comments:
# (1)   The backbone RNN code for IHC-LSTM
#
...
class Method(nn.Module):
    def __init__(self,ID, HD, dropout):
        ...
        self.WV_raw = nn.Parameter(torch.randn(HD, ID * 2))

        self.ih = nn.Linear(ID, HD * 3, bias = False)
    ...
    def forward(self, x, HC):
        ...
        for itr in range(tspan):
            ...
            D_kp, UH  = torch.matmul(Q_k, self.WV).chunk(2, dim = 1)
            D_k = torch.sigmoid(D_kp)
            V_k = (1 - D_k) * X_k +   D_k * UH

            Fi, Ii, Oi  = self.ih(V_k).chunk(3, dim = 1)
            Ai          = self.WA(X_k)

            F_k = torch.sigmoid(Fi + self.PFB)
            I_k = torch.sigmoid(Ii)
            A_k = Ai
            S_k = F_k * S_k + I_k * A_k
            
            O_k = torch.sigmoid(Oi)
