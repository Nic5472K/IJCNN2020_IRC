...
#---
# Comments:
# (1)   The backbone RNN code for IRC-FastGRNN
#
...
class Method(nn.Module):
    def __init__(self,ID, HD, dropout, l):
        ...
        self.hh_w_raw = nn.Parameter(torch.randn(HD, ID * 1) * 0.1)

        self.VB = nn.Parameter(torch.randn(ID) * 0.1)

        self.ih       = nn.Parameter(torch.randn(ID, HD) * 0.1)

        self.WA       = nn.Parameter(torch.randn(ID, HD) * 0.1)

        self.l  = l
 
        self.zeta   = nn.Parameter( torch.randn(HD) * 0.1 )
    ...
    def forward(self, x, HC):
        ...
        for itr in range(tspan):
            ...
            uVS_k       = torch.matmul(Q_k, self.hh_w)           

            V_k = X_k + torch.sigmoid(self.uV) * uVS_k            

            Fi = torch.matmul(V_k, self.ih)
            Ai = torch.matmul(X_k, self.WA)

            F_k = torch.sigmoid(Fi + self.PFB)

            I_k = torch.sigmoid(self.zeta) * (1 - F_k)            
